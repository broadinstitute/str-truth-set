...
def look_up_liftover_stats(prefix, total):
    INS_STRs_before_validation_step = int(re.search(
        f"{prefix}[ ]*([0-9,]+)[ ]* out of[ ]* {total:,d}.* INS variants", stepA_log_contents).group(1).replace(",", ""))

    DEL_STRs_before_validation_step = int(re.search(
        f"{prefix}[ ]*([0-9,]+)[ ]* out of[ ]* {total:,d}.* DEL variants", stepA_log_contents).group(1).replace(",", ""))

    multiallelic_INS_STRs_before_validation_step = int(re.search(
        f"{prefix}[ ]*([0-9,]+)[ ]* out of[ ]* {total:,d}.* multiallelic INS", stepA_log_contents).group(1).replace(",", ""))
    multiallelic_DEL_STRs_before_validation_step = int(re.search(
        f"{prefix}[ ]*([0-9,]+)[ ]* out of[ ]* {total:,d}.* multiallelic DEL", stepA_log_contents).group(1).replace(",", ""))
    mixed_multiallelic_STRs_before_validation_step = int(re.search(
        f"{prefix}[ ]*([0-9,]+)[ ]* out of[ ]* {total:,d}.* mixed multiallelic", stepA_log_contents).group(1).replace(",", ""))
...
liftover_failed_IndelStraddlesMultipleIntevals = int(
    re.search(f"([0-9,]+) IndelStraddlesMultipleIntevals", stepA_log_contents).group(1).replace(",", ""))

liftover_failed_for_other_reason = 0
liftover_failed_for_other_reason += int(
    re.search(f"([0-9,]+) CannotLiftOver", stepA_log_contents).group(1).replace(",", ""))
liftover_failed_for_other_reason += int(
    re.search(f"([0-9,]+) MismatchedRefAllele", stepA_log_contents).group(1).replace(",", ""))
liftover_failed_for_other_reason += int(
    re.search(f"([0-9,]+) NoTarget", stepA_log_contents).group(1).replace(",", ""))
...
total_STR_variants_after_1st_liftover_step = int(re.search(
    f"step3:pure_STR:output:[ ]*([0-9,]+)[ ]* TOTAL variants", stepA_log_contents).group(1).replace(",", ""))
...
DEL_variants_failed_hg38_to_t2t_liftover_due_to_IndelStraddlesMultipleIntevals = int(re.search(
    f"step3:pure_STR:failed-liftover:[ ]*([0-9,]+) out of .* filter:IndelStraddlesMultipleIntevals  DEL variants", stepA_log_contents).group(1).replace(",", ""))
multiallelic_DEL_variants_failed_hg38_to_t2t_liftover_due_to_IndelStraddlesMultipleIntevals = int(re.search(
    f"step3:pure_STR:failed-liftover:[ ]*([0-9,]+) out of .* filter:IndelStraddlesMultipleIntevals  multiallelic DEL variants", stepA_log_contents).group(1).replace(",", ""))
mixed_variants_failed_hg38_to_t2t_liftover_due_to_IndelStraddlesMultipleIntevals = int(re.search(
    f"step3:pure_STR:failed-liftover:[ ]*([0-9,]+) out of .* filter:IndelStraddlesMultipleIntevals  mixed multiallelic INS/DEL variants", stepA_log_contents).group(1).replace(",", ""))
assert re.search(
    f"step3:pure_STR:failed-liftover:[ ]*([0-9,]+) out of .* filter:IndelStraddlesMultipleIntevals  INS variants", stepA_log_contents) is None
INS_variants_failed_hg38_to_t2t_liftover_due_to_IndelStraddlesMultipleIntevals = 0
assert re.search(
    f"step3:pure_STR:failed-liftover:[ ]*([0-9,]+) out of .* filter:IndelStraddlesMultipleIntevals  multiallelic INS variants", stepA_log_contents) is None
...
INS_variants_failed_hg38_to_t2t_liftover = int(re.search(
    f"step3:pure_STR:failed-liftover:[ ]*([0-9,]+) out of .* filtered:  INS variants", stepA_log_contents).group(1).replace(",", ""))
DEL_variants_failed_hg38_to_t2t_liftover = int(re.search(
    f"step3:pure_STR:failed-liftover:[ ]*([0-9,]+) out of .* filtered:  DEL variants", stepA_log_contents).group(1).replace(",", ""))
multiallelic_INS_variants_failed_hg38_to_t2t_liftover = int(re.search(
    f"step3:pure_STR:failed-liftover:[ ]*([0-9,]+) out of .* filtered:  multiallelic INS variants", stepA_log_contents).group(1).replace(",", ""))
multiallelic_DEL_variants_failed_hg38_to_t2t_liftover = int(re.search(
    f"step3:pure_STR:failed-liftover:[ ]*([0-9,]+) out of .* filtered:  multiallelic DEL variants", stepA_log_contents).group(1).replace(",", ""))
mixed_multiallelic_INS_DEL_variants_failed_hg38_to_t2t_liftover = int(re.search(
    f"step3:pure_STR:failed-liftover:[ ]*([0-9,]+) out of .* filtered:  mixed multiallelic INS/DEL variants", stepA_log_contents).group(1).replace(",", ""))
...
kept_het_variants = int(re.search(
    f"([0-9,]+) .*kept variants: heterozygous reference genotype", stepA_log_contents).group(1).replace(",", ""))

kept_variants_insertions_that_match_adjacent_reference_sequence = int(re.search(
    f"([0-9,]+) .*kept variants: insertion matches the adjacent reference sequence", stepA_log_contents).group(1).replace(",", ""))

total_variants_passed_t2t_comparison = int(re.search(
    f"step4:pure_STR:output:[ ]*([0-9,]+)[ ]* TOTAL variants", stepA_log_contents).group(1).replace(",", ""))
...
liftover_failed__no_target = int(
    re.search(f"step5:pure_STR:failed-liftover:[ ]*([0-9,]+).*NoTarget", stepA_log_contents).group(1).replace(",", ""))

total_variants_passed_liftover_back_to_hg38 = int(re.search(
    f"step5:pure_STR:output:[ ]*([0-9,]+)[ ]* TOTAL variants", stepA_log_contents).group(1).replace(",", ""))
...
total_variants_with_different_position_after_hg38_to_t2t_to_hg38_liftovers = int(re.search(
    f"([0-9,]+) .*variants had a different position after hg38 => T2T => hg38", stepA_log_contents).group(1).replace(",", ""))
...
total_variants_passed_all_validation_steps = int(re.search(
    f"step7:pure_STR:output:[ ]*([0-9,]+)[ ]* TOTAL variants", stepA_log_contents).group(1).replace(",", ""))
...
INS_variants_in_final_truth_set = int(re.search(
    f"step7:pure_STR:output:[ ]*([0-9,]+) out of [ ]*{len(df_variants):,d}.*[%][)] INS variants", stepA_log_contents).group(1).replace(",", ""))
DEL_variants_in_final_truth_set = int(re.search(
    f"step7:pure_STR:output:[ ]*([0-9,]+) out of [ ]*{len(df_variants):,d}.*[%][)] DEL variants", stepA_log_contents).group(1).replace(",", ""))
multiallelic_INS_variants_in_final_truth_set = int(re.search(
    f"step7:pure_STR:output:[ ]*([0-9,]+) out of [ ]*{len(df_variants):,d}.*[%][)] multiallelic INS variants", stepA_log_contents).group(1).replace(",", ""))
multiallelic_DEL_variants_in_final_truth_set = int(re.search(
    f"step7:pure_STR:output:[ ]*([0-9,]+) out of [ ]*{len(df_variants):,d}.*[%][)] multiallelic DEL variants", stepA_log_contents).group(1).replace(",", ""))
mixed_multiallelic_INS_DEL_variants_in_final_truth_set = int(re.search(
    f"step7:pure_STR:output:[ ]*([0-9,]+) out of [ ]*{len(df_variants):,d}.*[%][)] mixed multiallelic INS/DEL variants", stepA_log_contents).group(1).replace(",", ""))
