...
def main():
    ...
    for batch_i in range(0, len(repeat_spec_file_stats_list), repeat_specs_per_machine):
        ...
        s1.command(f"""ls {repeat_spec_glob} | parallel --lb --jobs {repeat_specs_per_cpu * cpu_per_machine} \
            "GangSTR --ref {local_fasta} --bam {local_bam} --regions {{}} --out gangstr.{{/}} && python3.9 -m str_analysis.convert_gangstr_vcf_to_expansion_hunter_json gangstr.{{/}}.vcf"
        """)
        s1.command("ls -lhrt")

        s1.output(f"gangstr*.json", output_dir=os.path.join(output_dir, f"json"))
