...
do
    if [ $i -eq $lastJobIdx ]
    then
      tail -n ${tailNum} ${CODE_DIR}/markerInfo/${CHROM}markerInfo | cut -d ' ' -f 1-11,14- > currentMarkerInfo
      cat currentMarkerInfo
      ${CODE_DIR}/popSTR computeReadAttributes ${BAMLIST} ${CURR_DIR} currentMarkerInfo 8 135 ${CHROM} ${REFERENCE} ${CODE_DIR}/markerInfo/longRepeats N
    else

        headNum=`expr ${i} \* ${MARKERS_PER_JOB}`
        head -n ${headNum} ${CODE_DIR}/markerInfo/${CHROM}markerInfo | tail -n ${MARKERS_PER_JOB} | cut -d ' ' -f 1-11,14- > currentMarkerInfo
        cat currentMarkerInfo
        ${CODE_DIR}/popSTR computeReadAttributes ${BAMLIST} ${CURR_DIR} currentMarkerInfo 8 135 ${CHROM} ${REFERENCE} ${CODE_DIR}/markerInfo/longRepeats N
    fi
done
...
#run msGenotyper on all chromosomes using attributes and pnSlippage we now have
echo "Genotyping markers."
for ((i=1; i<=$lastJobIdx; i++))
do
    if [ $i -eq $lastJobIdx ]
    then
        ${CODE_DIR}/popSTR msGenotyperDefault -ADCN ${CURR_DIR}/attributes/${CHROM} -PNS pnSlippage -MS markerSlippage${CHROM} -VD ${CURR_DIR}/vcfs -VN ${CHROM} -ML <(cut -d ' ' -f 1,2,3,4,8,12,13 ${CODE_DIR}/markerInfo/${CHROM}markerInfo | tail -n ${tailNum}) -I ${lastJobIdx} -FP 1
    else
        headNum=`expr ${i} \* ${MARKERS_PER_JOB}`
        ${CODE_DIR}/popSTR msGenotyperDefault -ADCN ${CURR_DIR}/attributes/${CHROM} -PNS pnSlippage -MS markerSlippage${CHROM} -VD ${CURR_DIR}/vcfs -VN ${CHROM} -ML <(cut -d ' ' -f 1,2,3,4,8,12,13 ${CODE_DIR}/markerInfo/${CHROM}markerInfo | head -n ${headNum} | tail -n ${MARKERS_PER_JOB}) -I ${i} -FP 1
    fi
done
